...
CSub_Bottle::CSub_Bottle()
{
	...
	m_pAnimator->Play(L"SubWeapon_Bottle", false);
}
...
void CSub_Bottle::OnCollisionEnter(CCollider* _pOther)
{
	CSub::OnCollisionEnter(_pOther);
	CreateGlass();
}

void CSub_Bottle::Update()
{
	CSub::Update();
...
void CSub_Bottle::CreateGlass()
{
	...
	for (int i = 0; i < 20; ++i)
	{
		...
		dynamic_cast<CEffects_Glass*>(pObj)->SetGreen();
