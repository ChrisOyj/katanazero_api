...
void CScene_Stage2::Enter(bool _replay)
{
	if (!_replay)
	{
	...
	//오브젝트 로드
	CEditor::Get_Instance()->Set_Stage(2);
	...
	//Dragon
	pObj = CAbstractFactory<CMonster_Dragon>::Create();
	pObj->SetOwner(Get_Player());
	AddObject(pObj, GROUP_TYPE::DRAGON);

	pObj = CAbstractFactory<CDoor>::Create();
	pObj->SetOwner(Get_Player());
	AddObject(pObj, GROUP_TYPE::DOOR);

	//Portal
	pObj = CAbstractFactory<CPortal>::Create(1070.f, 640.f, GROUP_TYPE::MONSTER);
